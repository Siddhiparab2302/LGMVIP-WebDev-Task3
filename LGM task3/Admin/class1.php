    ...
    <table class="table">
            ...
            <tbody>
               <?php while($row = $result->fetch_assoc()){ 
                echo '<tr>';
                    echo'<th scope="row">'.$row['class_id'].'</th>';
                    echo'<td>'.$row['class_name'].'</td>';
                    echo'<td>';
                    echo'
                    <form action="editclass.php" method="POST" class="d-inline">
...
if(isset($_REQUEST['delete'])){
    $sql = "DELETE FROM class WHERE class_id = {$_REQUEST['id']} ";
...
<div>
    <a class="btn btn-danger box" href="./addclass.php"><i class="fas fa-plus fa-2x"></i></a>
</div>
</div>
